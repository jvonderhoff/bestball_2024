...
def update_labels():
    ...
    for i, values in enumerate(selected_values):
        if i < len(label_list):
            ...
            team_stack = [ 
                values[2], values[3]]
            team_stack = f"{values[3]} {values[4]}"
            team_stack_list[i].config(text=team_stack)
...
for _ in range(20):
    ...
    team_stack_label = Label(team_stack_data_frame, text="")
    team_stack_label.pack(side="top")
    team_stack_list.append(team_stack_label)
