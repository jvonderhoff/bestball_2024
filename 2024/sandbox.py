...
def review_new_lineup():
    ...
    # Create The Treeview
    lineup_treeview = ttk.Treeview(add_lineup, yscrollcommand=tree_scroll.set, selectmode=EXTENDED)
    lineup_treeview.grid(row=0, column=0, sticky="nsew")

    tree_scroll.config(command=lineup_treeview.yview)
...
search_frame.grid(padx=10, pady=10, row=3, column=1)

# Add player name search entry box
name_search_entry = Entry(search_frame)
name_search_entry.grid(padx=20, pady=20)


# Add player name search entry box
team_search_entry = Entry(search_frame)
team_search_entry.grid(padx=20, pady=20)
...
lineup_data_frame.grid(padx=20, row=5, column=0)

team_stack_data_frame = LabelFrame(root, text="Teaam Stack")
team_stack_data_frame.grid(padx=20, row=5, column=4)

contest_data_label = Label(root, text="") 
...
# Set focus to the Treeview widget after the frame is displayed
tree_frame.bind("<Map>", lambda event: my_tree.focus_set())
